...
class Controller:
    ...
    def __init__(self, game: ext_elev.GameAPI):
        ...
        self._collapse_moves = _single_passenger and _all_reliable
        ...
        self._use_expectimax = _rl_like or (_full_range and _multi)
    ...
    def choose_next_action(self, state):
        ...
        # Farming problems -> exact expectimax (already optimal for them).
        if self._use_expectimax:
            return self._expectimax_action(state, steps_left)

        # Plan-and-execute: a cached deterministic A* plan delivers everyone;
        # we just emit its next action. On a failure the realised state differs,
        # so the lookup naturally re-plans (a cache hit once seen).
        ps = (state[0], state[1])
        plan = self._get_plan(ps)
        if plan and len(plan) <= steps_left:
            return plan[0]

        # Endgame (plan no longer fits) or a planning failure -> expectimax.
        return self._expectimax_action(state, steps_left)
